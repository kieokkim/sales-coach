...
| 버전 | 테마 | 핵심 내용 |
|------|------|-----------|
| v1.3 | 대화형 분석 | 자연어 질의 → DB 조회 → LLM 답변 |
| v1.4 | 고객 코호트 분석 | 신규/재구매 비율, 교차구매 패턴 추적 |
| v1.5 | 할인 민감도 분석 | 할인율-판매량 탄력성, 프로모션 ROI(마진 손실 vs 매출 증가) |
| v1.6 | Eval 레이어 | insight/action 출력 품질을 사람 라벨과 비교 검증 |
| v1.7 | 재고 시그널 | 판매 속도 기반 소진 예측, 발주 타이밍 알림 |
| v2.0 | 아키텍처 전환 | RAG(과거 인사이트 검색) + FastAPI 분리 + Langfuse Observability |
| v2.1 | 외부 신호 통합 | 채널/매장 간 카니발리제이션 탐지, 날씨 연동 |

**설계 원칙**: LLM은 판단(insight)·도출(action)·서술(commentary) 3개 노드만 담당합니다.
수치 계산과 비교 기준값은 전부 rule-based로 처리하므로,
